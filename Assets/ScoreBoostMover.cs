...
public class ScoreBoostMover : MonoBehaviour
{
    public float speed = 4f;
    public List<Sprite> sprites; // List of sprites to choose from
    public List<int> scoreValues = new List<int>() { 2, 3, 4 }; // Score increments
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        if (other.gameObject.CompareTag("Player"))
        {
            ...
            int audioSourceIndex = 2; // Assuming this index is correctly set for the desired sound
            ...
            if (spriteRenderer != null && sprites.Contains(spriteRenderer.sprite))
            {
                ...
                {
                    ScoreManager.AddScore(scoreValues[index]);
                }
            }
            else
            {
